...
class index extends React.Component {
    ...
    pagename=""
	links = [['App','/content/classrooms',''],['Articles','/content/articles','active']]
                            ...
                            <Col md={12}>
                                <div className="fright">
                                    <ul className="vurox-horizontal-links vurox-standard-ul pt-3">
										<li className="p-0"><Link href={{pathname:'/content/articles/add'}} shallow><a><i className="ti-plus"></i>&nbsp;Tambah artikel</a></Link></li>
                                    </ul>
                                </div>
                            </Col>
                        </Row>
											...
											<tbody>
												{
													this.props.articles.list.map(item=>(
														<tr key={item.id}>
															<td><Checkbox/></td>
															<td valign="middle"><Link href={{pathname:'/content/articles/[id]',query:{id:item.id}}} shallow><a>{item.name}</a></Link></td>
															<td valign="middle"><Link href={{pathname:'/content/category/[id]',query:{id:item.category.id}}} shallow><a>{item.category.name}</a></Link></td>
															<td valign="middle">
																{
																	item.tags.map(tag=>
																		<Tag key={tag.id}>
