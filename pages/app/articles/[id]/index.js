...
const Index = props => {
    ...
    const links = [['App','/app/classrooms',''],['Articles','/app/articles',''],[item.name,`/app/articles/${item.id}`,'active']]
                                ...
                                <Col md={12}>
                                    <div className="fright">
                                        <ul className="vurox-horizontal-links vurox-standard-ul">
                                            <li className="p-0"><Link href={{pathname:'/app/articles/[id]/edit',query:{id:item.id}}} shallow><Button className="link" type="link" size="small" icon={<i className="ti-pencil"></i>}>&nbsp;Edit artikel</Button></Link></li>
                                        </ul>
                                    </div>
                                </Col>
                            </Row>
                                        ...
                                        <table className="table table-borderless mb-0">
                                            ...
                                            <tbody>
                                            {
                                                props.comments.list.map(item=>(
                                                    <tr key={item.id}>
                                                        <td><Checkbox/></td>
                                                        <td valign="middle"><Link href={{pathname:'/app/articles/[id]/comments/[cid]',query:{id:item.id,cid:item.id}}} shallow><a>{item.name}</a></Link></td>
